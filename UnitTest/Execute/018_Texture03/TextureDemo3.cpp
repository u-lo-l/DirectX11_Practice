...
namespace Sdt
{
	void TextureDemo3::Initialize()
	{
		shader = new Shader(L"14_TextureAddress.fx");
	...
	void TextureDemo3::Tick()
	{
		MainCamera->Tick();

		ImGui::SliderInt("Pass", reinterpret_cast<int*>(&Pass), 0, 2);
		ImGui::SliderInt("Address", reinterpret_cast<int*>(&Address), 0, 3);
		ImGui::SliderFloat("Tiling", &Tiling, 0.3f, 32);
	...
	void TextureDemo3::Render()
	{
		...
		CHECK(shader->AsScalar("Address")->SetInt(Address) >= 0);
		CHECK(shader->AsScalar("Tiling")->SetFloat(Tiling) >= 0);
		CHECK(shader->AsScalar("Offset")->SetFloat(Offset) >= 0);
